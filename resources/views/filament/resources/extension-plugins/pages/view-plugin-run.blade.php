        ...
        <section class="overflow-hidden rounded-3xl border border-gray-200/80 bg-gradient-to-br from-white via-primary-50/20 to-white shadow-sm dark:border-gray-800 dark:from-gray-950 dark:via-primary-950/20 dark:to-gray-950">
            <div class="grid gap-6 px-6 py-6 lg:grid-cols-[minmax(0,1.2fr)_minmax(280px,0.8fr)] lg:px-8">
                <div class="space-y-4">
                    ...
                    <div class="grid gap-3 sm:grid-cols-3">
                        <div class="rounded-2xl border border-gray-200 bg-white/80 p-4 backdrop-blur dark:border-gray-800 dark:bg-gray-900/80">
                            <div class="text-[11px] font-semibold uppercase tracking-[0.18em] text-gray-500 dark:text-gray-400">Invocation</div>
                            <div class="mt-2 text-sm font-medium text-gray-950 dark:text-white">{{ $run->action ? \Illuminate\Support\Str::headline($run->action) : ($run->hook ? \Illuminate\Support\Str::headline($run->hook) : 'Plugin run') }}</div>
                            <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ \Illuminate\Support\Str::headline($run->invocation_type) }}</div>
                        </div>
                        <div class="rounded-2xl border border-gray-200 bg-white/80 p-4 backdrop-blur dark:border-gray-800 dark:bg-gray-900/80">
                            <div class="text-[11px] font-semibold uppercase tracking-[0.18em] text-gray-500 dark:text-gray-400">Current signal</div>
                            <div class="mt-2 text-sm font-medium text-gray-950 dark:text-white">{{ $run->progress_message ?: ($latestMessage ?: 'No log messages yet') }}</div>
                            ...
                            <div class="mt-2 text-xs text-gray-500 dark:text-gray-400">{{ $progress }}% recorded progress.</div>
                        </div>
                        <div class="rounded-2xl border border-gray-200 bg-white/80 p-4 backdrop-blur dark:border-gray-800 dark:bg-gray-900/80">
                ...
                <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
                    ...
                    <div class="rounded-2xl border border-gray-200 bg-white/90 p-5 shadow-xs dark:border-gray-800 dark:bg-gray-900/90">
                        <div class="text-[11px] font-semibold uppercase tracking-[0.18em] text-gray-500 dark:text-gray-400">Returned totals</div>
                        <div class="mt-4 flex flex-wrap gap-2 text-sm text-gray-600 dark:text-gray-300">
                            ...
                            @empty
                                <span class="text-sm text-gray-500 dark:text-gray-400">This run did not publish aggregate totals.</span>
                            @endforelse
                        </div>

                        @if($reportPath)
                            <div class="mt-4 rounded-2xl bg-gray-50 p-4 text-xs text-gray-500 dark:bg-gray-950/60 dark:text-gray-300">
        ...
        <section class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
            <div class="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <div class="flex items-center justify-between gap-3">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Latest persisted log messages for this run.</p>
                    </div>
                </div>
                ...
                @if($this->logs->isNotEmpty())
                    <div class="mt-5 space-y-3">
                        @foreach($this->logs as $log)
                            <div class="rounded-2xl border border-gray-200 bg-gray-50/70 p-4 dark:border-gray-800 dark:bg-gray-950/60">
                                <div class="flex flex-wrap items-center gap-2">
                                    <span class="inline-flex rounded-full bg-gray-900 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-white dark:bg-gray-100 dark:text-gray-900">
                                        {{ strtoupper($log->level ?? 'info') }}
                                    </span>
                                    <span class="text-xs text-gray-500 dark:text-gray-400">{{ optional($log->created_at)->toDateTimeString() }}</span>
                                </div>
                                <p class="mt-3 text-sm text-gray-800 dark:text-gray-100">{{ $log->message }}</p>
                                @if(! empty($log->context))
                                    <pre class="mt-3 overflow-x-auto rounded-xl bg-white p-3 text-xs text-gray-700 dark:bg-gray-900 dark:text-gray-200">{{ json_encode($log->context, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) }}</pre>
                                @endif
                            </div>
                        @endforeach
                    </div>
            ...
            <div class="space-y-6">
                <div class="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    ...
                    <pre class="mt-4 overflow-x-auto rounded-2xl bg-gray-950 p-4 text-xs text-gray-100">{{ $payload ?: '{}' }}</pre>
                </div>

                <div class="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    <h2 class="text-sm font-semibold text-gray-950 dark:text-white">Result</h2>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">The persisted result payload returned by the plugin.</p>
                    <pre class="mt-4 overflow-x-auto rounded-2xl bg-gray-950 p-4 text-xs text-gray-100">{{ $result ?: '{}' }}</pre>
                </div>

                <div class="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    <h2 class="text-sm font-semibold text-gray-950 dark:text-white">Run state</h2>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Checkpoint data used for long-running or resumed work.</p>
                    <pre class="mt-4 overflow-x-auto rounded-2xl bg-gray-950 p-4 text-xs text-gray-100">{{ $runState ?: '{}' }}</pre>
                </div>
            </div>
        </section>
    </div>
